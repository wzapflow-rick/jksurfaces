      ...
      {products.map((product, index) => (
        <article key={product.sku} className={`product-card group ${index % 2 === 1 ? 'md:mt-24' : ''}`}>
          <div className="relative aspect-[1.05] overflow-hidden bg-ink">
            <Image src={product.image} alt={product.name} fill className="object-cover transition duration-700 group-hover:scale-105" sizes="(min-width: 768px) 50vw, 100vw" />
            <span className="absolute left-5 top-5 text-[9px] uppercase tracking-[.24em] text-paper/65">Seleção JK</span>
          </div>
