...
export function BrandLogos() {
  return (
    <section aria-labelledby="marcas-title" className="border-y border-white/10 bg-ink px-5 py-8 md:px-10 md:py-10">
      <p id="marcas-title" className="eyebrow mb-6 text-center text-[10px]">Marcas que você encontra na JK</p>
        ...
        <div className="brand-logos-track flex w-max items-center">
          {[0, 1].map((group) => (
            <ul key={group} className="flex shrink-0 items-center" aria-hidden={group === 1 ? true : undefined}>
              {filled.map((logo, i) => (
                <li key={`${group}-${i}`} className="flex w-[120px] items-center justify-center md:w-[150px]">
                  {/* Logos monocromáticas para tratamento uniforme na faixa. */}
                  <img
                    src={logo.src || '/placeholder.svg'}
                    alt={group === 1 ? '' : logo.alt}
                    className="h-5 w-auto select-none object-contain opacity-45 transition-opacity duration-300 hover:opacity-100 md:h-6"
                    draggable={false}
                  />
                </li>
              ))}
