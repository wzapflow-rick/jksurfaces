...
export function SiteHeader() {
  ...
  return <header className="fixed inset-x-0 top-0 z-50 border-b border-white/10 bg-[#080b0f]/75 px-5 py-4 backdrop-blur-md md:px-10">
    <div className="mx-auto flex max-w-7xl items-center justify-between">
      <a href="#top" aria-label="JK Surfaces — início"><Image src={logo} alt="JK Surfaces" width={180} height={70} className="h-10 w-auto object-contain object-left md:h-12" priority /></a>
      <nav className="hidden items-center gap-8 text-[10px] uppercase tracking-[.24em] text-white/70 md:flex" aria-label="Navegação principal"><a className="transition-colors hover:text-[#c7b79b]" href="/produtos">Produtos</a><a className="transition-colors hover:text-[#c7b79b]" href="#curadoria">Curadoria</a><a className="transition-colors hover:text-[#c7b79b]" href="#contato">Contato</a></nav>
    ...
    {open && <nav id="mobile-menu" className="mx-auto flex max-w-7xl flex-col gap-5 border-t border-white/10 py-5 text-[11px] uppercase tracking-[.24em] text-white/75 md:hidden"><a href="/produtos" onClick={() => setOpen(false)}>Produtos</a><a href="#curadoria" onClick={() => setOpen(false)}>Curadoria</a><a href="#contato" onClick={() => setOpen(false)}>Contato</a></nav>}
  </header>
}
